...
parser.add_argument('-p', '--num_processes', type=int, help='number of processor', default=4)
...
train_file = "{}/train.csv".format(args.input_root, args.num_classes)
val_file = "{}/val.csv".format(args.input_root, args.num_classes)
test_file = "{}/test.csv".format(args.input_root, args.num_classes)
...
def load_test_video_list(file_path):
    ...
    with open(file_path) as f:
        for line in f.readlines():
            ...
            if line == "":
                continue
            _, youtube_id, start_time, end_time, temp, _ = line.split(",")
            if temp.strip() == 'split':
                continue
            video_id = "{}_{:06d}_{:06d}".format(youtube_id, int(start_time), int(end_time))
